...
import { formatDate, formateMobile, formatMoney } from "@/utils";
import { Descriptions, Modal } from "antd";
import { useImperativeHandle, useState } from "react";

export default function OrderDetai(props: IDetailProp) {
	...
	return (
		<Modal title='订单详情' width={800} open={visible} footer={false} onCancel={handleCancel}>
			<Descriptions column={2} style={{ padding: '10px 30px' }}>
				...
				<Descriptions.Item label='下单用户'>{detail?.orderId}</Descriptions.Item>
        ...
        <Descriptions.Item label='订单金额'>{formatMoney(detail?.orderAmount??0)}</Descriptions.Item>
        <Descriptions.Item label='用户支付金额'>{formatMoney(detail?.userPayAmount??0)}</Descriptions.Item>
        <Descriptions.Item label='司机到账金额'>{formatMoney(detail?.driverAmount??0)}</Descriptions.Item>
