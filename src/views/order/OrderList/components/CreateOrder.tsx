...
export default function CreateOrder(props: IModalProp) {
	...
	return (
		<Modal title='创建订单' width={800} open={visible} okText='确定' cancelText='取消' onOk={handleOk} onCancel={handleCancel}>
			<Form form={form} layout="horizontal" labelAlign="right" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
				...
				<Row>
					...
					<Col span={12}>
						<Form.Item name='userPayAmount' label='支付金额' rules={[{ required: true, message: '请输入支付金额' }]}>
							<Input type='number' placeholder="请输入下单支付金额" />
						</Form.Item>
					</Col>
				</Row>
				...
				<Row>
					...
					<Col span={12}>
						<Form.Item name='driverAmount' label='司机金额' rules={[{ required: true, message: '请选输入司机金额' }]}>
							<Input type='number' placeholder="请输入下单司机金额" />
						</Form.Item>
					</Col>
				</Row>
